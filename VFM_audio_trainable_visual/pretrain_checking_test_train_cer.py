...
from models.av_net_multifusion_trainable_frontends_conformer_diff_fusion import AVNetMultiFusion
...
for i in num_of_words:
    ...
    # scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args["NUM_STEPS"], eta_min=args["FINAL_LR"], last_epoch=- 1, verbose=False)
    loss_function = nn.CTCLoss(blank=0, zero_infinity=False)


    # removing the checkpoints_conformer_diff_fusion directory if it exists and remaking it
    if os.path.exists(args["CODE_DIRECTORY"] + "/checkpoints_conformer_diff_fusion"):
        while True:
            break
            ch = input("Continue and remove the 'checkpoints_conformer_diff_fusion' directory? y/n: ")
            if ch == "y":
                break
            elif ch == "n":
                exit()
                ...
                print("Invalid input")
        shutil.rmtree(args["CODE_DIRECTORY"] + "/checkpoints_conformer_diff_fusion")

    os.mkdir(args["CODE_DIRECTORY"] + "/checkpoints_conformer_diff_fusion")
    os.mkdir(args["CODE_DIRECTORY"] + "/checkpoints_conformer_diff_fusion/models")
    os.mkdir(args["CODE_DIRECTORY"] + "/checkpoints_conformer_diff_fusion/plots")


    #loading the pretrained weights
    if args["PRETRAINED_MODEL_FILE"] is not None:
        print("\n\nPre-trained Model File: %s" %("/pretrain_weights/0"+str(i)+"/best_till_now.pt"))
        print("\nLoading the pre-trained model .... \n")
        model.load_state_dict(torch.load(args["CODE_DIRECTORY"] + "/pretrain_weights/0"+str(i)+"/best_till_now.pt", map_location=device))
        model.to(device)
        print("Loading Done.\n")
    ...
    for step in range(args["NUM_STEPS"]):
        ...
        file.write("pretrain words "+str(i)+' train_cer '+trainingCER+' test_cer '+validationCER)
        ...
        scheduler.step(trainingCER)


        #saving the model weights and loss/metric curves in the checkpoints_conformer_diff_fusion directory after every few steps
        if 1:#((step%args["SAVE_FREQUENCY"] == 0) or (step == args["NUM_STEPS"]-1)) and (step != 0):

            savePath = args["CODE_DIRECTORY"] + "/checkpoints_conformer_diff_fusion/models/pretrain_{:03d}w-step_{:04d}-cer_{:.3f}.pt".format(args["PRETRAIN_NUM_WORDS"],
                                                                                                                        step, trainingCER)
            ...
            plt.legend()
            plt.savefig(args["CODE_DIRECTORY"] + "/checkpoints_conformer_diff_fusion/plots/pretrain_{:03d}w-step_{:04d}-loss.png".format(args["PRETRAIN_NUM_WORDS"], step))
            plt.close()
            ...
            plt.legend()
            plt.savefig(args["CODE_DIRECTORY"] + "/checkpoints_conformer_diff_fusion/plots/pretrain_{:03d}w-step_{:04d}-wer.png".format(args["PRETRAIN_NUM_WORDS"], step))
            plt.close()
