...
#removing the checkpoints_loaded directory if it exists and remaking it
if os.path.exists(args["CODE_DIRECTORY"] + "/checkpoints_loaded"):
    while True:
        ch = input("Continue and remove the 'checkpoints_loaded' directory? y/n: ")
        if ch == "y":
            break
        elif ch == "n":
            exit()
            ...
            print("Invalid input")
    shutil.rmtree(args["CODE_DIRECTORY"] + "/checkpoints_loaded")

os.mkdir(args["CODE_DIRECTORY"] + "/checkpoints_loaded")
os.mkdir(args["CODE_DIRECTORY"] + "/checkpoints_loaded/models")
os.mkdir(args["CODE_DIRECTORY"] + "/checkpoints_loaded/plots")
...
visual_frontend_dict = torch.load("../pretrained/visual_frontend.pt")

pretrained_dict = {'visual_frontend.'+k: v for k, v in visual_frontend_dict.items()}
...
for step in range(args["NUM_STEPS"]):
    ...
    #saving the model weights and loss/metric curves in the checkpoints_loaded directory after every few steps
    if 1:#((step%args["SAVE_FREQUENCY"] == 0) or (step == args["NUM_STEPS"]-1)) and (step != 0):

        savePath = args["CODE_DIRECTORY"] + "/checkpoints_loaded/models/pretrain_{:03d}w-step_{:04d}-wer_{:.3f}.pt".format(args["PRETRAIN_NUM_WORDS"],
                                                                                                                    step, validationWER)
        ...
        plt.legend()
        plt.savefig(args["CODE_DIRECTORY"] + "/checkpoints_loaded/plots/pretrain_{:03d}w-step_{:04d}-loss.png".format(args["PRETRAIN_NUM_WORDS"], step))
        plt.close()
        ...
        plt.legend()
        plt.savefig(args["CODE_DIRECTORY"] + "/checkpoints_loaded/plots/pretrain_{:03d}w-step_{:04d}-wer.png".format(args["PRETRAIN_NUM_WORDS"], step))
        plt.close()
