...
class AVNetMultiFusion(nn.Module):
    ...
    def __init__(self, dModel, nHeads, numLayers, peMaxLen, inSize, fcHiddenSize, dropout, numClasses):
        ...
        self.audioGating = nn.Sequential(ConformerBlock(dim = dModel, dim_head = 64, heads = 8, ff_mult = 4, conv_expansion_factor = 2,
                                           conv_kernel_size = 7, attn_dropout = dropout, ff_dropout = dropout, conv_dropout = dropout))
        self.videoGating = nn.Sequential(ConformerBlock(dim = dModel, dim_head = 64, heads = 8, ff_mult = 4, conv_expansion_factor = 2,
                                           conv_kernel_size = 7, attn_dropout = dropout, ff_dropout = dropout, conv_dropout = dropout))
    ...
    def forward(self, inputBatch):
        ...
        if (audioBatch is not None) and (videoBatch is not None):
            ...
            gatedAudio = self.audioGating(audioBatch)
            gatedVideo = self.videoGating(videoBatch)
            #print(gatedAudio.shape, videoBatch.shape)
            gatedAudio = torch.sigmoid(gatedAudio)*videoBatch.transpose(0,1)
            gatedVideo = torch.sigmoid(gatedVideo)*audioBatch.transpose(0,1)
            ...
            #print(audioBatchExt.shape, gatedVideo.shape, videoBatchExt.shape, gatedAudio.shape)    
            jointBatch = torch.cat([audioBatchExt, gatedVideo.transpose(0,1), videoBatchExt.transpose(0,1), gatedAudio], dim=3)
