...
class AVNetMultiFusion(nn.Module):
    ...
    def __init__(self, dModel, nHeads, numLayers, peMaxLen, inSize, fcHiddenSize, dropout, numClasses):
        ...
        self.fusion_1 = nn.Linear(2*dModel,8*dModel)
        self.fusion_batchnorm = nn.BatchNorm1d(8*dModel)
        self.fusion_2 = nn.Linear(8*dModel,dModel)
    ...
    def forward(self, inputBatch):
        ...
        if videoInputBatch is not None:
            ...
            if videoInputBatch.shape[1]*640 != audioInputBatch.shape[1]:
        ...
        if videoBatch == None:
            videoBatch = torch.zeros(audioBatch.shape,device='cuda')
            # videoBatch = videoBatch.transpose(0,1)
        if audioBatch == None:
            audioBatch = torch.zeros(videoBatch.shape,device='cuda')
        ...
        if (audioBatch is not None) and (videoBatch is not None):
            ...
            #### new fusion
            if videoBatch.shape[0]!=batch_size:
                videoBatch = videoBatch.transpose(0,1)
            if audioBatch.shape[0] != batch_size:
                audioBatch = audioBatch.transpose(0,1)
            # print(audioBatch.shape,videoBatch.shape)
            # print(audioBatch.shape,videoBatch.shape)
            fused = torch.cat([audioBatch,videoBatch],dim=2)
            # print(fused.shape,fused.device)
            fused = self.fusion_1(fused).transpose(1,2)
            # print(fused.shape,fused.device)
            fused = self.fusion_batchnorm(fused).relu().transpose(1,2)
            # print(fused.shape,fused.device)
            fused = self.fusion_2(fused)
            jointBatch = fused
