...
if os.path.exists(args["CODE_DIRECTORY"] + "/checkpoints_1"):
    while True:
        ...
        if ch == "y":
            ...
            print("Invalid input")
    shutil.rmtree(args["CODE_DIRECTORY"] + "/checkpoints_1")

os.mkdir(args["CODE_DIRECTORY"] + "/checkpoints_1")
os.mkdir(args["CODE_DIRECTORY"] + "/checkpoints_1/models")
os.mkdir(args["CODE_DIRECTORY"] + "/checkpoints_1/plots")
...
for step in range(args["NUM_STEPS"]):
    ...
    if 1:#((step%args["SAVE_FREQUENCY"] == 0) or (step == args["NUM_STEPS"]-1)) and (step != 0):
        ...
        savePath = args["CODE_DIRECTORY"] + "/checkpoints_1/models/pretrain_{:03d}w-step_{:04d}-wer_{:.3f}.pt".format(args["PRETRAIN_NUM_WORDS"],
                                                                                                                    step, validationWER)
        ...
        plt.legend()
        plt.savefig(args["CODE_DIRECTORY"] + "/checkpoints_1/plots/pretrain_{:03d}w-step_{:04d}-loss.png".format(args["PRETRAIN_NUM_WORDS"], step))
        plt.close()
        ...
        plt.legend()
        plt.savefig(args["CODE_DIRECTORY"] + "/checkpoints_1/plots/pretrain_{:03d}w-step_{:04d}-wer.png".format(args["PRETRAIN_NUM_WORDS"], step))
        plt.close()
