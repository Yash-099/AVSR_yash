...
from data.lrs2_dataset import LRS2Main, LRS2Pretrain
...
trainData = LRS2Pretrain("pretrain", args["DATA_DIRECTORY"], args["PRETRAIN_NUM_WORDS"], args["CHAR_TO_INDEX"], args["STEP_SIZE"], audioParams, videoParams, noiseParams)
...
valData = LRS2Pretrain("val", args["DATA_DIRECTORY"], args["MAIN_REQ_INPUT_LENGTH"], args["CHAR_TO_INDEX"], args["STEP_SIZE"], audioParams, videoParams, noiseParams)
...
for name,parameter in model.named_parameters():
    if parameter.requires_grad:
        ...
        print(name,'this is not trainable')
model.to(device)
...
if os.path.exists(args["CODE_DIRECTORY"] + "/base_checkpoints"):
    while True:
        ch = input("Continue and remove the 'base_checkpoints' directory? y/n: ")
        if ch == "y":
            break
        elif ch == "n":
            exit()
            ...
            print("Invalid input")
    shutil.rmtree(args["CODE_DIRECTORY"] + "/base_checkpoints")

print(args["CODE_DIRECTORY"] + "/base_checkpoints")

os.makedirs(args["CODE_DIRECTORY"] + "/base_checkpoints")
os.mkdir(args["CODE_DIRECTORY"] + "/base_checkpoints/models")
os.mkdir(args["CODE_DIRECTORY"] + "/base_checkpoints/plots")

print("hello yash Bhavsar")
...
unfreezeParams = [model.jointConv.parameters(), model.jointDecoder.parameters(), model.outputConv.parameters()] 
...
for step in range(args["NUM_STEPS"]):
    ...
    if args["SAVE WEIGHT FILES"]:
        if ((validationWER <= minValWER) or (step%args["SAVE_FREQUENCY"] == 0) or (step == args["NUM_STEPS"]-1)) and (step != 0):
            ...
            savePath = args["CODE_DIRECTORY"] + "/base_checkpoints/models/train-step_{:04d}-wer_{:.3f}.pt".format(step, validationWER)
            ...
            plt.legend()
            plt.savefig(args["CODE_DIRECTORY"] + "/base_checkpoints/plots/train-step_{:04d}-loss.png".format(step))
            plt.close()
            ...
            plt.legend()
            plt.savefig(args["CODE_DIRECTORY"] + "/base_checkpoints/plots/train-step_{:04d}-wer.png".format(step))
            plt.close()
